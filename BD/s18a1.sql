...
CREATE DATABASE IF NOT EXISTS commerce_tech
...
CREATE TABLE IF NOT EXISTS vendas (
  id_venda     INT PRIMARY KEY AUTO_INCREMENT,  -- Chave primária
  data_venda   DATE         NOT NULL,           -- Data da venda
  produto      VARCHAR(100) NOT NULL,           -- Nome do produto
  vendedor     VARCHAR(100) NOT NULL,           -- Nome do vendedor
  quantidade   INT          NOT NULL CHECK (quantidade > 0),    -- Qtde vendida
...
INSERT INTO vendas (data_venda, produto, vendedor, quantidade, valor_total) VALUES
('2025-07-01', 'Fone Bluetooth A10',  'Ana',   2,  299.80),
('2025-07-03', 'Teclado Mecânico K87','Bruno', 1,  399.90),
('2025-07-05', 'Mouse Gamer X5',      'Carla', 3,  357.00),
('2025-07-08', 'Monitor 27\" IPS',    'Diego', 1,  1399.00),
('2025-07-08', 'Fone Bluetooth A10',  'Bruno', 1,  149.90),
('2025-07-10', 'Cadeira Office Pro',  'Ana',   1,  899.00),
('2025-07-12', 'Teclado Mecânico K87','Carla', 2,  799.80),
('2025-07-13', 'Webcam 1080p',        'Diego', 2,  259.80),
('2025-07-15', 'Mouse Gamer X5',      'Ana',   1,  119.00),
('2025-07-16', 'Monitor 27\" IPS',    'Bruno', 2,  2798.00),
('2025-07-18', 'Cadeira Office Pro',  'Carla', 1,  899.00),
('2025-07-20', 'Hub USB-C 7x1',       'Diego', 3,  447.00),
('2025-07-21', 'Fone Bluetooth A10',  'Ana',   4,  599.60),
('2025-07-23', 'Webcam 1080p',        'Bruno', 1,  129.90),
('2025-07-24', 'Hub USB-C 7x1',       'Carla', 1,  149.00),
('2025-07-25', 'Monitor 27\" IPS',    'Ana',   1,  1399.00);
...
SELECT
  ...
  produto,
  vendedor,
  ROW_NUMBER() OVER (ORDER BY data_venda DESC, id_venda DESC) AS numero_venda
FROM vendas
...
SELECT
  ...
  produto,
  vendedor,
  ROW_NUMBER() OVER (PARTITION BY vendedor ORDER BY data_venda DESC, id_venda DESC) AS numero_venda_por_vendedor
FROM vendas
ORDER BY vendedor, data_venda DESC, id_venda DESC;
...
WITH ranking_prod AS (
  SELECT
    ...
    quantidade,
    valor_total,
    ROW_NUMBER() OVER (PARTITION BY produto ORDER BY valor_total DESC, data_venda DESC, id_venda DESC) AS rn
  FROM vendas
)

SELECT id_venda, data_venda, produto, vendedor, quantidade, valor_total FROM ranking_prod
WHERE rn = 1
ORDER BY produto;
